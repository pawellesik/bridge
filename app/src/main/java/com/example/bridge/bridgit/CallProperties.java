...
public class CallProperties extends CallFeature {
    ...
    public CallProperties(Call call, Function<PositionState, PositionCalls> partnerBids, 
                          boolean forcing1Round, boolean forcingToGame, 
                          boolean agreeTrump, Suit trump, Constraint.StaticConstraint... constraints) {
        ...
        if (agreeTrump) {
            if (call instanceof Call.Bid) {
                this.trumpSuit = ((Call.Bid) call).getSuit();
            }
        } else if (trump != null) {
    ...
    public Function<PositionState, PositionCalls> getPartnerBids() { return partnerBids; }

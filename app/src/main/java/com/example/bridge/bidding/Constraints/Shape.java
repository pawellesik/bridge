...
public class Shape {
    ...
    /**
     * Pokazuje partnerowi informację o minimalnej liczbie kart w pozostałych kolorach.
     */
    public static class ShowsOthersShape extends HandConstraint implements IShowsHand, IDescribeConstraint {
        private final Suit skipSuit;
        private final int min;

        public ShowsOthersShape(Suit skipSuit, int min) {
            this.skipSuit = skipSuit;
            this.min = min;
        }

        private Suit getSkipSuit(Call call, PositionState ps) {
            if (skipSuit != null) return skipSuit;
            Suit last = ps.getPairState().getLastShownSuit();
            if (last != null) return last;
            if (call instanceof Bid) return ((Bid) call).getSuit();
            return null;
        }

        @Override
        public boolean conforms(Call call, PositionState ps, HandSummary hs) {
            Suit skip = getSkipSuit(call, ps);
        ...
        public void showHand(Call call, PositionState ps, HandSummary.ShowState showHand) {
            ...
            for (Suit s : Suit.values()) {
                if (s != skip) {
                    showHand.getSuits().get(s).showShape(min, 13);
                }
            }
        }

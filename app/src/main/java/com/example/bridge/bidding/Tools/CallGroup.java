            ...
            if (feature instanceof CallFeatureGroup) {
                ...
                if (getPositionState().isValidNextCall(feature.getCall()) && !positionCalls.containsKey(feature.getCall())) {
                    if (feature.satisfiesStaticConstraints(getPositionState())) {
                        ...
                        // If this feature is a BidRule, check if the private hand matches it
                        if (feature instanceof BidRule && getPositionState().privateHandConforms((BidRule) feature)) {
                            details.addMatchedRule((BidRule) feature);
                            if (bestCall == null) {
                                bestCall = details;
                            }
                        }
